-- Fix: Make handle_pos_journal_entry calculate COGS from transaction items
...
BEGIN
  ...
  -- First try to get from metadata (for compatibility)
  IF NEW.metadata IS NOT NULL AND NEW.metadata ? 'total_cogs' THEN
    v_total_cogs := (NEW.metadata->>'total_cogs')::NUMERIC;
  ELSE
    -- Calculate COGS from items using FIFO deduction
    FOR v_item IN SELECT * FROM jsonb_array_elements(NEW.items)
    LOOP
      -- Try to deduct from FIFO layers and sum up COGS
      -- Use exception handling to gracefully handle missing layers
      BEGIN
