-- Drop and recreate the trigger function to handle refunds properly
...
BEGIN
  ...
  INSERT INTO journal_entries (
    ...
    v_abs_total + v_total_cogs + v_abs_discount,
    v_abs_subtotal + v_abs_tax + v_total_cogs,
    NEW.total,  -- Store actual transaction amount (can be negative for refunds)
    'posted',
    NEW.cashier_id,
    NEW.cashier_id,
    NOW()
  ...
  IF v_is_refund THEN
    ...
    -- Refund: Credit discount (reverses discount given)
...
CREATE TRIGGER pos_journal_entry_trigger
