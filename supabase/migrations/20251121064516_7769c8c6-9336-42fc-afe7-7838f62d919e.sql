-- Fix create_cash_register_opening_entry to handle NULL account gracefully
...
BEGIN
  ...
  IF NEW.opening_cash > 0 AND TG_OP = 'INSERT' THEN
    ...
    -- If no cash account found, skip journal entry creation entirely
    IF v_cash_account_id IS NULL THEN
      RAISE WARNING 'Cash account (1010 or 1110) not found, skipping journal entry creation for cash register opening';
      RETURN NEW;
    END IF;
    ...
    ELSE
      -- If no owner account, use a generic equity account
      SELECT id INTO v_owner_account_id 
      FROM accounts 
