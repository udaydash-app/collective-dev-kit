-- Fix journal entry balance calculation for POS transactions with discounts
...
BEGIN
  ...
  ELSE
    ...
    LOOP
      ...
      EXCEPTION
        WHEN OTHERS THEN
          -- If FIFO deduction fails, use estimated COGS (70% of selling price)
  ...
  -- The key: total_debit = total_credit
  -- Debit side: Payment account + COGS
  -- Credit side: Sales + Tax + Inventory - Discount (discount reduces credit, so we add it back)
  INSERT INTO journal_entries (
    ...
    -- Debit: Payment received + COGS
    NEW.total + v_total_cogs,
    -- Credit: Sales + Tax + COGS - Discount (since discount is debited, we add it to balance)
  ...
  -- Sales Discount (this is a debit that reduces revenue)
