-- Enhanced stock restoration trigger with better logging and error handling
...
BEGIN
  ...
  LOOP
    ...
    v_variant_id := v_item.value->>'variant_id';
    v_product_id := v_item.value->>'product_id';
    
    RAISE NOTICE 'Processing item - Product: %, Variant: %, Quantity: %', v_product_id, v_variant_id, v_quantity;
    
    -- Restore stock to variant if variant_id exists and is not empty
    IF v_variant_id IS NOT NULL AND v_variant_id != '' AND v_variant_id != 'null' THEN
      ...
      END IF;
    -- Otherwise restore to product if product_id exists
    ELSIF v_product_id IS NOT NULL AND v_product_id != '' THEN
      ...
      END IF;
    ELSE
      RAISE WARNING '⚠️ No valid product_id or variant_id for item';
    END IF;
  END LOOP;
