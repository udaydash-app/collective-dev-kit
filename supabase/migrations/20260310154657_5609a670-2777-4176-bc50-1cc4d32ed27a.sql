...
-- 1. Ensure a "Cash Over/Short" account exists (SYSCOHADA code 7799)
INSERT INTO public.accounts (account_code, account_name, account_type, current_balance, is_active)
SELECT '7799', 'Cash Over/Short', 'revenue', 0, true
WHERE NOT EXISTS (SELECT 1 FROM public.accounts WHERE account_code = '7799');

-- 2. Update the closing trigger to post cash_difference
...
BEGIN
  ...
  IF NEW.status = 'closed' AND OLD.status = 'open' AND NEW.closing_cash IS NOT NULL AND NEW.closing_cash > 0 THEN
    ...
    -- Get the discrepancy (actual - expected)
    v_difference := COALESCE(NEW.cash_difference, 0);
    ...
    -- Handle discrepancy if there is one and the account exists
    IF v_difference != 0 AND v_over_short_account_id IS NOT NULL THEN
      IF v_difference > 0 THEN
        ...
        INSERT INTO journal_entry_lines (journal_entry_id, account_id, description, debit_amount, credit_amount)
        VALUES (v_journal_entry_id, v_owner_cash_account_id, 'Cash overage', v_difference, 0);
        INSERT INTO journal_entry_lines (journal_entry_id, account_id, description, debit_amount, credit_amount)
      ...
      ELSE
        ...
        INSERT INTO journal_entry_lines (journal_entry_id, account_id, description, debit_amount, credit_amount)
        VALUES (v_journal_entry_id, v_over_short_account_id, 'Cash short - missing cash from register', ABS(v_difference), 0);
        INSERT INTO journal_entry_lines (journal_entry_id, account_id, description, debit_amount, credit_amount)
        VALUES (v_journal_entry_id, v_owner_cash_account_id, 'Cash shortage', 0, ABS(v_difference));
        -- Keep the entry balanced
        UPDATE journal_entries
