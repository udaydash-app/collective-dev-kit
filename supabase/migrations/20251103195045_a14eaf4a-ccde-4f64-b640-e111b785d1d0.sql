-- Fix trigger to use truly unique account codes with gen_random_uuid()
...
BEGIN
  ...
  IF NEW.is_customer = true THEN
    -- Generate unique customer account code using gen_random_uuid()
  ...
  IF NEW.is_supplier = true THEN
    -- Generate unique supplier account code using gen_random_uuid()
    v_unique_suffix := UPPER(SUBSTRING(REPLACE(gen_random_uuid()::text, '-', '') FROM 1 FOR 10));
