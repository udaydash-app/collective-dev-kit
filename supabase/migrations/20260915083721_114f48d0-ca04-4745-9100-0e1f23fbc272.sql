...
BEGIN
  IF TG_OP = 'DELETE' THEN
    DELETE FROM public.journal_entries WHERE reference = 'EXP-' || OLD.id::text;
    RETURN OLD;
  END IF;
  ...
  IF NEW.account_id IS NOT NULL THEN
    ...
    WHERE id = NEW.account_id AND is_active = true LIMIT 1;
  END IF;

  IF v_expense_account_id IS NULL THEN
    SELECT id INTO v_expense_account_id FROM public.accounts
    WHERE account_type = 'expense' AND is_active = true
      AND (account_name ILIKE '%' || NEW.category || '%' OR account_code LIKE '6%')
    ORDER BY CASE WHEN account_name ILIKE '%' || NEW.category || '%' THEN 0 ELSE 1 END, account_code
    LIMIT 1;
  END IF;

  IF v_expense_account_id IS NULL THEN
    SELECT id INTO v_expense_account_id FROM public.accounts
    WHERE account_type = 'expense' AND is_active = true ORDER BY account_code LIMIT 1;
  END IF;
  ...
  -- Prefer the explicitly chosen "paid from" account
  IF NEW.paid_from_account_id IS NOT NULL THEN
    SELECT id, 'Paiement - ' || account_name INTO v_payment_account_id, v_payment_desc
    FROM public.accounts WHERE id = NEW.paid_from_account_id AND is_active = true LIMIT 1;
  END IF;
  ...
  IF v_payment_account_id IS NULL THEN
    ...
    END CASE;
    v_payment_desc := CASE NEW.payment_method
  ...
  INSERT INTO public.journal_entries (
    ...
    'EXP-' || NEW.id::text,
    NEW.amount, NEW.amount, NEW.amount, 'posted',
  ...
  INSERT INTO public.journal_entry_lines (journal_entry_id, account_id, description, debit_amount, credit_amount)
  VALUES (v_journal_entry_id, v_payment_account_id, v_payment_desc, 0, NEW.amount);
