-- Drop and recreate the trigger to handle UPDATE operations
DROP TRIGGER IF EXISTS create_contact_ledger_accounts_trigger ON contacts;

-- Modify the function to handle both INSERT and UPDATE
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    -- Handle customer opening balance update
    IF NEW.is_customer = true AND NEW.customer_ledger_account_id IS NOT NULL THEN
      IF OLD.opening_balance IS DISTINCT FROM NEW.opening_balance THEN
        -- Delete old opening balance journal entry
        DELETE FROM journal_entries 
        WHERE reference LIKE 'OB-CUST-%' 
          AND description = 'Opening Balance - ' || OLD.name
          AND EXISTS (
        ...
        -- Create new journal entry if balance > 0
    ...
    -- Handle supplier opening balance update
