-- Fix the trigger to properly delete opening balance entries for contacts that are both customer and supplier
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    IF NEW.is_customer = true AND NEW.customer_ledger_account_id IS NOT NULL THEN
      IF OLD.opening_balance IS DISTINCT FROM NEW.opening_balance THEN
        -- Delete old opening balance journal entry - improved matching
