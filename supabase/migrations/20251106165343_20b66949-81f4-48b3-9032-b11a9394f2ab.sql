-- Update supplier payment trigger to handle updates
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    ...
    WHERE reference = 'SUPP-PAY-' || OLD.id::text;
    RETURN OLD;
  END IF;
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    SET
      ...
      description = 'Supplier Payment - ' || (SELECT name FROM contacts WHERE id = NEW.supplier_id),
      total_debit = NEW.amount,
      total_credit = NEW.amount,
      updated_at = now()
    WHERE reference = 'SUPP-PAY-' || NEW.id::text
    RETURNING id INTO v_je_id;

    -- Get account IDs
    SELECT supplier_ledger_account_id INTO v_supplier_account_id
    FROM contacts WHERE id = NEW.supplier_id;
  ...
  IF v_supplier_account_id IS NULL THEN
    RAISE EXCEPTION 'Supplier ledger account not found for supplier %', NEW.supplier_id;
  END IF;
  ...
  VALUES (
    NEW.payment_date,
    'Supplier Payment - ' || (SELECT name FROM contacts WHERE id = NEW.supplier_id),
    'SUPP-PAY-' || NEW.id::text,
    NEW.amount,
    NEW.amount,
    'posted',
    now(),
...
-- Update payment receipt trigger to handle updates
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    ...
    DELETE FROM journal_entries 
    WHERE reference = 'PAY-RCPT-' || OLD.id::text;
    RETURN OLD;
  END IF;
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    UPDATE journal_entries
    SET
      entry_date = NEW.receipt_date,
      description = 'Payment Receipt - ' || (SELECT name FROM contacts WHERE id = NEW.customer_id),
      total_debit = NEW.amount,
      total_credit = NEW.amount,
      updated_at = now()
    WHERE reference = 'PAY-RCPT-' || NEW.id::text
    RETURNING id INTO v_je_id;

    -- Get account IDs
    SELECT customer_ledger_account_id INTO v_customer_account_id
    FROM contacts WHERE id = NEW.customer_id;
  ...
  IF v_customer_account_id IS NULL THEN
    RAISE EXCEPTION 'Customer ledger account not found for customer %', NEW.customer_id;
  END IF;
  ...
  VALUES (
    NEW.receipt_date,
    'Payment Receipt - ' || (SELECT name FROM contacts WHERE id = NEW.customer_id),
    'PAY-RCPT-' || NEW.id::text,
    NEW.amount,
    NEW.amount,
    'posted',
    now(),
