...
export const WalkieTalkieButton = () => {
    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button
          ...
          size="sm"
          title="Walkie-Talkie"
          className={cn("h-7 w-7 p-0", isPlaying && "animate-pulse")}
        >
          <Radio className={cn("h-3 w-3", isPlaying ? "text-accent" : "text-muted-foreground")} />
          {isPlaying && (
      ...
      <PopoverContent className="w-72 p-0" align="end">
        {/* Header */}
        <div className="p-3 border-b border-border bg-primary text-primary-foreground flex items-center justify-between rounded-t-md">
        ...
        <div className="p-3 space-y-3">
          <div className="text-center text-xs text-muted-foreground">{deviceName}</div>
          ...
          >
            <div className="flex flex-col items-center gap-2">
              ...
              <span className="text-sm">{isRecording ? 'RELEASE TO SEND' : 'HOLD TO TALK'}</span>
            </div>
          </Button>
