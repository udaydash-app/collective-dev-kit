  ...
  return (
    <>
      <PageHeader
        ...
        actions={<Button onClick={() => navigate("/ledgerly/bills/new")}><Plus className="h-4 w-4 mr-1.5" />New Bill</Button>}
      />
      <div className="p-6 space-y-4">
        ...
        <Card className="shadow-[var(--shadow-card)]">
          <Table>
            ...
            <TableBody>
              {filtered.map((r) => (
                <TableRow key={r.id} className="cursor-pointer hover:bg-muted/40" onClick={() => navigate(`/ledgerly/bills/${r.id}`)}>
                  <TableCell className="font-medium">
                    <Link to={`/ledgerly/bills/${r.id}`} onClick={(e) => e.stopPropagation()} className="text-primary hover:underline">{r.bill_number}</Link>
                  </TableCell>
                  ...
                  <TableCell className="text-right" onClick={(e) => e.stopPropagation()}>
                    <div className="flex items-center justify-end gap-1">
                      <Button variant="ghost" size="icon" title="Edit" onClick={() => navigate(`/ledgerly/bills/${r.id}`)}>
