...
const BillPrint = () => {
  ...
  const handleDownload = async () => {
    if (!sheetRef.current || !bill) return;
    setDownloading(true);
    try {
      await downloadElementAsPdf(sheetRef.current, `bill-${bill.bill_number}.pdf`);
    } catch (e: any) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [pRes, bRes, lRes] = await Promise.all([
        ...
        supabase.from("bills").select("*, contact:contacts(name, email, phone, address)").eq("id", id).single(),
        supabase.from("bill_lines").select("id, description, quantity, rate, amount, item:items(name, sku, unit)").eq("bill_id", id).order("created_at"),
      ]);
      if (bRes.error || !bRes.data) { toast.error(bRes.error?.message ?? "Bill not found"); setLoading(false); return; }
      ...
      setBill(b);
      setContact(b.contact ?? null);
      setLines((lRes.data ?? []) as unknown as BillLine[]);
      setLoading(false);
    })();
  }, [id]);

  if (loading) return <div className="p-10 text-center text-sm text-muted-foreground">Loading…</div>;
  if (!bill) return <div className="p-10 text-center text-sm text-muted-foreground">Not found.</div>;
    ...
    <div className="min-h-screen bg-muted/30 print:bg-white">
      ...
      <div className="print:hidden border-b bg-card">
        <div className="max-w-4xl mx-auto px-6 py-3 flex items-center justify-between">
          <Button variant="outline" size="sm" onClick={() => navigate(`/ledgerly/bills/${bill.id}`)}>
            <ArrowLeft className="h-4 w-4 mr-2" />Back to bill
          </Button>
        ...
        <div ref={sheetRef} className="bg-card text-card-foreground shadow-sm print:shadow-none p-10 print:p-0 invoice-sheet">
          ...
          <div className="flex items-start justify-between mb-10 gap-6">
            ...
            <div className="text-right shrink-0">
              <h2 className="text-3xl font-bold uppercase tracking-tight text-primary">Bill</h2>
              <p className="text-sm font-medium mt-1 num">#{bill.bill_number}</p>
              <p className="text-xs text-muted-foreground mt-2 capitalize">Status: {bill.status}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-8 mb-10">
            <div>
              <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold mb-2">Bill from</p>
            ...
            <div className="text-right space-y-2">
              <div>
                <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Bill date</p>
              ...
              {bill.due_date && (
                <div>
                  ...
                  <p className="text-sm font-medium">{formatDate(bill.due_date)}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex justify-end mb-10">
            <div className="w-72 space-y-2 text-sm">
              <div className="flex justify-between py-1">
                ...
                <span className="num font-medium">{formatMoney(bill.subtotal, currency)}</span>
              </div>
              {Number(bill.tax_amount) > 0 && (
                <div className="flex justify-between py-1">
                  <span className="text-muted-foreground">Tax ({formatNumber(bill.tax_percent, 2)}%)</span>
                  <span className="num font-medium">{formatMoney(bill.tax_amount, currency)}</span>
                </div>
              )}
              <div className="flex justify-between py-2 border-t-2 border-foreground/80 text-base font-bold">
                <span>Total</span>
                <span className="num">{formatMoney(bill.total, currency)}</span>
              </div>
              {Number(bill.paid_amount) > 0 && (
                <>
                  <div className="flex justify-between py-1">
                    <span className="text-muted-foreground">Paid</span>
                    <span className="num font-medium">{formatMoney(bill.paid_amount, currency)}</span>
                  </div>
