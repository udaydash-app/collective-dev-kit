  ...
  const exportBalancesPDF = async () => {
    ...
    autoTable(doc, {
      ...
      body: rows.map((r) => [
        ...
        r.phone || '-',
        formatCurrencyPdf(r.credit_limit),
        formatCurrencyPdf(Math.abs(Number(r.balance))),
        Number(r.balance) >= 0 ? 'Receivable' : 'Payable',
      ]),
      foot: [['TOTAL', '', '', formatCurrencyPdf(netBalance), netBalance >= 0 ? 'Receivable' : 'Payable']],
  ...
  const exportAgingPDF = async () => {
    ...
    autoTable(doc, {
      ...
      body: agingRows.map((r) => [
        ...
        ...BUCKET_KEYS.map((k) => formatCurrencyPdf(r.buckets[k])),
        formatCurrencyPdf(r.total),
      ]),
      foot: [[
        'TOTAL', '', '', '', '',
        ...BUCKET_KEYS.map((k) => formatCurrencyPdf(aging.totals[k])),
        formatCurrencyPdf(aging.totals.total),
      ]],
      styles: { fontSize: 8, cellPadding: 1.5 },
      headStyles: { fillColor: [34, 197, 94] },
      footStyles: { fillColor: [30, 41, 59], textColor: 255 },
