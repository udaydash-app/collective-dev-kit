...
class KioskPrintService {
  ...
  private generateReceiptHTML(data: KioskReceiptData): string {
    ...
    return `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>@page{size:80mm auto;margin:0}html,body{width:80mm}*{margin:0;padding:0;box-sizing:border-box}body{font-family:monospace;font-size:12px;line-height:1.35;color:#000;background:#fff;padding:2mm 3mm 4mm 3mm;overflow:hidden}.center{text-align:center}.bold{font-weight:bold}.large{font-size:18px}.small{font-size:12px}.mb-1{margin-bottom:4px}.mb-2{margin-bottom:8px}.mt-1{margin-top:4px}.mt-2{margin-top:8px}.border-t{border-top:1px solid #000;padding-top:8px}.border-b{border-bottom:1px solid #000;padding-bottom:8px}.dashed{border-style:dashed}.row{display:flex;justify-content:space-between;margin-bottom:4px}.item{margin-bottom:6px;page-break-inside:avoid}.item-name{font-weight:bold;word-wrap:break-word}.item-details{display:flex;justify-content:space-between;font-size:12px}.strike{text-decoration:line-through}img{max-height:80px;max-width:60mm;width:auto;display:block;margin:0 auto 1px}.paper-cut{margin-top:3mm;border-top:1px dashed #000;padding-top:2mm;text-align:center;font-size:10px}</style></head><body><div class="center mb-2">${data.logoUrl ? `<img src="${data.logoUrl}" alt="Logo">` : ''}<div class="bold large">${data.storeName || 'GLOBAL INDIAN MART'}</div><div class="small">Fresh groceries delivered to your doorstep</div><div class="small mt-2">Transaction: ${data.transactionNumber}</div><div class="small">${formatDateTime(data.date)}</div><div class="small bold mt-2 mb-2">Customer: ${data.customerName || 'Walk-in Customer'}</div>${data.customerPhone ? `<div class="small mb-1">Phone: ${data.customerPhone}</div>` : ''}</div><div class="border-t border-b mb-2">${data.items.map(item => {
