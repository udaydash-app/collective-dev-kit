...
class KioskPrintService {
  ...
  private generateReceiptHTML(data: KioskReceiptData): string {
    ...
    }).join('')}</div><div class="mb-2"><div class="row"><span>Subtotal:</span><span>${formatCurrency(data.subtotal)}</span></div><div class="row"><span>Tax (15%):</span><span>${formatCurrency(data.tax)}</span></div>${data.discount && data.discount > 0 ? `<div class="row"><span>Discount:</span><span>-${formatCurrency(data.discount)}</span></div>` : ''}<div class="row bold large border-t" style="padding-top:4px"><span>TOTAL:</span><span>${formatCurrency(data.total)}</span></div></div><div class="border-t mb-2"><div>Payment Method: ${data.paymentMethod.toUpperCase()}</div></div>${data.customerBalance !== undefined && data.customerBalance !== null ? `<div class="border-t dashed mb-2"><div class="bold">${data.isUnifiedBalance ? 'Unified Balance:' : 'Current Balance:'} ${formatCurrency(data.customerBalance)}</div>${data.isUnifiedBalance ? '<div class="small mt-1">(Combined customer & supplier account)</div>' : ''}</div>` : ''}<div class="center small"><div>Thank you for shopping with us!</div>${data.supportPhone ? `<div class="mt-2">For support: ${data.supportPhone}</div>` : ''}</div><script>window.print();</script></body></html>`;
  }
}
